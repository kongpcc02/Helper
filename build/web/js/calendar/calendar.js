	...
	if (mon) {
		...
		if (year) {
			...
			if (year.year != date.getFullYear()) {
				if(year.year-date.getFullYear()!=543){
					date.setFullYear(year.year-543);
					cal.setDate(date);
					cal.dateClicked = false;
					cal.callHandler();
				}
...
Date.prototype.print = function (str) {
	...
	s["%y"] = ('' + (y+543)).substr(2, 2); // year without the century (range 00 to 99)
	s["%Y"] = y+543;		// year with the century
